...
def parse_matched_annotation_file(path: str) -> Tuple[List[List[float]], List[int]]:
    ...
    first row is the old box, second row is the new box. We keep both boxes so the model
    learns to predict the moved object in its initial and final positions from the pixel diff.
    ...
    for i in range(0, len(lines), 2):
        # guard for incomplete pairs
        if i >= len(lines):
            break
        # initial box
        init_parts = lines[i].split()
        if len(init_parts) >= 6:
            _, x, y, w, h, cls_id = init_parts[:6]
        ...
        if i + 1 < len(lines):
            ...
            if len(final_parts) >= 6:
                _, x, y, w, h, cls_id = final_parts[:6]
                x, y, w, h = map(float, (x, y, w, h))
                cls_id = int(cls_id)
                boxes.append([x, y, x + w, y + h])
                labels.append(cls_id)
